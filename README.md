...
`marc-andreessen-persona`, `resolve-tickets`, and `plan-reviewer` are intended to be **explicit-invoke only** so *you* control when they apply — they have side effects (committing) or a strong voice override, so they should never fire on their own.
