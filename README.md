...
`worktree`, `close-issue`, `fan-out`, and `marc-andreessen-persona` are **explicit-invoke only** (`disable-model-invocation: true`) so *you* control when they apply — three of them have side effects (creating worktrees, closing issues) or a strong voice override, so they should never fire on their own. `issue-break-up` auto-triggers when a GitHub issue looks too big for one shot.
