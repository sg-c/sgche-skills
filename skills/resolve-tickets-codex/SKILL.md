...
1. From the invocation directory, require a Git worktree and determine its GitHub repository identity. Verify `gh auth status`; fetch the parent issue body, comments, state, and native sub-issue and blocking links from that repository. Parent must be open and reachable.
...
7. Build the dependency DAG only from each child's tracker-native blocking links, using the relationships published and verified by `$mattpocock-skills:to-tickets`. Do not require or interpret a textual dependency field. Record external blockers. Fail when native blocking-link data is inaccessible or malformed, on cycles, when a linked in-batch child is omitted, or when an external blocker remains open.
